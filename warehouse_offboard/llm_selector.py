...
class TargetSelector:
    ...
    def select_target_rule_based(self, user_input: str) -> Optional[str]:
        ...
        # ── 1. 정확한 패턴 매칭: A-01, A02, A-04 등 ──
        match = re.fullmatch(r'A-?0*([1-9]\d*)', text)
        if match:
            candidate = f'A-{int(match.group(1)):02d}'
            if candidate in self.available_targets:
                return candidate

        # ── 2. 임베디드 패턴: "A-01로 가", "A04 가줘" ──
        embedded = re.search(r'A-?0*([1-9]\d*)', text)
        if embedded:
            candidate = f'A-{int(embedded.group(1)):02d}'
            if candidate in self.available_targets:
                return candidate

        # ── 3. 숫자만 입력: "1", "2", "3", "4" ──
        match_num = re.fullmatch(r'0*([1-9]\d*)', text)
        if match_num:
            candidate = f'A-{int(match_num.group(1)):02d}'
            if candidate in self.available_targets:
                return candidate

        # ── 4. 한국어 매핑 ──
        korean_map = {
            '1번': 'A-01', '1번째': 'A-01', '첫번째': 'A-01', '첫째': 'A-01', '하나': 'A-01',
            '2번': 'A-02', '2번째': 'A-02', '두번째': 'A-02', '둘째': 'A-02', '둘': 'A-02',
            '3번': 'A-03', '3번째': 'A-03', '세번째': 'A-03', '셋째': 'A-03', '셋': 'A-03',
            '4번': 'A-04', '4번째': 'A-04', '네번째': 'A-04', '넷째': 'A-04', '넷': 'A-04',
        }
        ...
        # ── 5. 영어 서수 매핑 ──
        english_map = {
            'FIRST': 'A-01', 'SECOND': 'A-02', 'THIRD': 'A-03', 'FOURTH': 'A-04',
        }
        upper_raw = raw.upper()
